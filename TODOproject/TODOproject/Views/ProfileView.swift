...
struct ProfileView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                Section(header: Text("Системные интеграции")) {
                    HStack {
                        ...
                        if isLoadingSteps {
                            ...
                            Text(todaySteps.map(String.init) ?? "Нет доступа")
                                .foregroundColor(.secondary)
                        }
                    }

                    Button("Обновить данные Здоровья") {
                        Task {
                            await refreshHealthData()
                        }
                    }

                    Button("Открыть Будильник") {
                        Task { @MainActor in
    ...
    private func refreshHealthData() async {
        ...
        let steps = await SystemIntegrationManager.shared.fetchTodayStepCount(requestAccessIfNeeded: true)
        todaySteps = steps.map { Int($0.rounded()) }
