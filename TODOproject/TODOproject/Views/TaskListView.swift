    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let progress = taskVM.stepProgress(for: task) {
                    let progressValue = progress.requiredDays > 0
                        ? min(1.0, Double(progress.achievedDays) / Double(progress.requiredDays))
                        : 0
                    Text("Шаги сегодня: \(progress.todaySteps)/\(progress.goal)")
                        .font(.caption)
                        .foregroundColor(progress.isTodayCompleted ? .green : .orange)
                    Text("Дней с выполненной нормой: \(progress.achievedDays)/\(progress.requiredDays)")
                        .font(.caption2)
                        .foregroundColor(.secondary)
